...
def main():
    ...
    def submit1():
        ...
        k=textBox3.get()
        outputfile=textBox1.get()+'/result_K-means_'+str(k)+'_'+str(maxiterations)+'.csv'
        otc=textBox1.get()+'/centers_K-means_'+str(k)+'_'+str(maxiterations)+'.csv'
        if(inputfile=='' or outputfile=='' or maxiterations=='' or k==''):
            ...
            for i in f:
                ...
                data.append(j[1:])
            X = np.array(data)
            kmeans = KMeans(n_clusters=int(k),init='random',max_iter=int(maxiterations)).fit(X)
            ...
            for j in kmeans.cluster_centers_:
                ...
                co+=1
            oc.close()
        my_label4=Label(root,text='Within Set Squared Error:')
