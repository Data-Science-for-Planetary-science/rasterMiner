...
def main():
    ...
    def submit1():
        ...
        k=textBox3.get()
        outputfile=textBox1.get()+'/result_fuzzy C-means_'+str(k)+'_'+str(maxiterations)+'.csv'
        otc=textBox1.get()+'/centers_fuzzy C-means_'+str(k)+'_'+str(maxiterations)+'.csv'
        if(inputfile=='' or outputfile=='' or maxiterations=='' or k==''):
            ...
            for i in f:
                ...
                data.append(j[1:])
            X = np.array(data)
            cmeans = FuzzyKMeans(k=int(k), m=2,max_iter=int(maxiterations))
            cmeans.fit(X)
            wr=cmeans.labels_

            # WSSE=cmeans.inertia_
            # wr = kmeans.predict(X)
            ...
            of.close()
            co=1
            for j in cmeans.cluster_centers_:
                text='Center-'+str(co)
                for d in j:
                    text+=','+str(d)
                # print(text)
                ...
                co+=1
            oc.close()
        my_label4=Label(root,text='output file created successfully')
        my_label4.grid(column=0, row=10)
    root=Tk()
    root.title('fuzzy c-means')
    v = IntVar(root, 1) 
